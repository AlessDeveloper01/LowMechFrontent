...
const displayName = computed(() => {
  ...
  return `${auth.user.name} ${auth.user.apellido}`.trim()
})
      ...
      <section class="relative group">
        ...
        >
          <header class="px-5 pb-3 mb-3 border-b border-border dark:border-dk-border">
            ...
            <span
              v-if="auth.user?.rol"
              class="inline-block mt-1 px-2 py-0.5 text-[1rem] font-semibold rounded bg-primary/10 text-primary capitalize"
            >
              {{ auth.user.rol }}
            </span>
          </header>
